...
<body class="bg-gray-100 text-gray-800 font-sans">
    ...
    <div class="container mx-auto px-4">
        ...
        <form action="{{ route('monuments.update', $monument->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto mb-6">
            ...
            <div class="mb-6">
                <label for="intervals" class="block text-lg font-semibold mb-2 text-gray-700">Tipi:</label>
                <select name="intervals[]" id="intervals" multiple class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-150 ease-in-out">
                    ...
                    <option value="{{ $interval->id }}" class="py-2 px-4 hover:bg-gray-100" {{ in_array($interval->id, $curIntervals) ? 'selected' : '' }}>{{ $interval->title }}</option>
                    @endforeach
                </select>
            </div>
